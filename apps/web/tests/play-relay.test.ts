...
function setup(gasMode:'usdc'|'eth'='eth') {
  let now=1000000, allowance=0n, gate:Promise<void>|undefined, startError:Error|undefined;
  ...
  return{setStartError:(error:Error)=>{startError=error;},connect,phone,tablet,calls,fixture,close:()=>relay.close(),setAllowance:(value:bigint)=>{allowance=value;},advance:()=>{now+=180001;},setGate:(value:Promise<void>)=>{gate=value;}};
}
