...
test('inventory reads wallet and slot at the same block, includes unconfigured collection IDs and preserves missing data',async()=>{
  let unavailable=false,expanded=false,blockReads=0;const functions:string[]=[];
  const client={
    ...
    readContract:async({functionName,blockNumber}:{functionName:string;blockNumber:bigint})=>{
      ...
      if(functionName==='getPrizeCatalog')return expanded?[[12,13,14],[6n,7n,8n].map(tokenId=>({kind:2,token:BASE_PRIZE_COLLECTION,tokenId}))]:[[3],[{kind:2,token:BASE_PRIZE_COLLECTION,tokenId:1n}]];
  ...
  for(const [symbol,id] of [[12,'6'],[13,'7'],[14,'8']] as const){const nft=inventory.nfts.find(n=>n.symbol===symbol)!;assert.equal(nft.tokenId,id);assert.equal(nft.balance,'7');assert.equal(nft.canDeposit,false);}
  expanded=true;const configured=await read(wallet,slot);
