...
function Fixture(){
  ...
  return <main className="admin-main" style={{margin:0,padding:24}}><button onClick={()=>setLocked(!locked)}>Toggle owner</button><output data-testid="submitted">{last}</output><h1>Wallet e riserve</h1><AdminInventory inventory={inventory} slot={null} busy={false} loadError="" canDeposit onReload={()=>{}} onConfigure={()=>{}} onOperation={async(action,args)=>{setLast(JSON.stringify({action,args}));}}/></main>;
}
