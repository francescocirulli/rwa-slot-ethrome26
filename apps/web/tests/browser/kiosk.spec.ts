...
test('demo never contacts real relay, covers both stages, ignores repeated lever and resets to real',async({page})=>{
  ...
  await expect(page.locator('#game-title')).toContainText('You won', {timeout:12000});
  ...
  await page.locator('#settings-open').click();await page.locator('#mode-switch').click();await expect(page).toHaveURL(/\/$/);await expect(page.locator('body')).toHaveAttribute('data-mode','real');
});
