...
const weights=[[94,62],[73,48],[48,32],[45,30],[38,26],[38,26],[38,26],[30,20],[0,100],[0,67],[0,45],[0,13]];
...
test('all 1000 rolls reproduce the configured weights and 40.4/49.5/10.1 totals',()=>{
  ...
  assert.deepEqual(totals,[404,495,101]);assert.equal(counts.get('255-0'),101);
...
test('every outcome on every payline has unique columns and exactly its one winning line',()=>{
  for(let roll=0;roll<1000;roll++)for(let line=0;line<3;line++){
    ...
    assert.equal(grid.length,15);assert.ok(grid.every((s:number)=>Number.isInteger(s)&&s>=0&&s<12));
...
test('forced outcomes reject impossible prizes and pay exactly the configured reward',()=>{
  for(const bad of ['four','jackpot','8-3','9-3','10-3','11-3','12-5','0-4'])assert.throws(()=>model.draw(bad));
  assert.equal(model.draw('loss').payout,null);
  for(let symbol=0;symbol<12;symbol++)for(const match of [3,5]){
    ...
    else if([0,3,8,9,10].includes(symbol)){assert.equal(p.kind,2);assert.equal(p.amount,'1');assert.equal(p.tokenId,({0:'5',3:'1',8:'2',9:'3',10:'4'} as Record<number,string>)[symbol]);}
...
test('random mode draws one outcome and an independent uniform line',()=>{
  for(const value of [0,0.34,0.99]){const samples=[0.999,value];const g=model.draw('random',()=>samples.length?samples.shift():0.5);assert.equal(g.winningSymbol,11);assert.equal(g.matchCount,5);assert.equal(g.winningLine,Math.floor(value*3));}
});
