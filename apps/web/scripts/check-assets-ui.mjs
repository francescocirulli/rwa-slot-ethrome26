...
const inventory={address:wallet,contract:null,eth:'0.01',block:'123',updatedAt:Date.now(),swapEnabled:true,assets,nfts:[{symbol:0,name:'Magnet'},{symbol:3,name:'Gadget'},{symbol:8,name:'ENS Registration'},{symbol:9,name:'Urbe Hub Day Pass'},{symbol:10,name:'T-shirt'}].map(n=>({...n,token:null,tokenId:null,balance:null})),freeSpins:null};
...
try{
  ...
  assert.equal(await page.locator('.asset-card').count(),8);assert.equal(await page.locator('.collectible-card').count(),6);assert.equal(await page.getByRole('button',{name:'Deposit into the slot'}).isEnabled().catch(()=>false),false);
  ...
  await page.getByRole('button',{name:'Swap',exact:false}).first().click();await page.getByLabel('USDC amount to swap').fill('10');await page.getByRole('button',{name:'Request quote'}).click();await page.getByRole('heading',{name:'Autorizza USDC per lo swap'}).waitFor();assert.equal(sends,0);assert.match(await page.getByLabel('Saldi del wallet condiviso').innerText(),/ETH\s*0\.01/);
  await page.screenshot({path:'artifacts/admin-swap-ipad.png',fullPage:true});
  await page.getByRole('button',{name:'Approve USDC with Privy'}).click();await page.getByRole('heading',{name:'USDC approved.'}).waitFor({timeout:15000});assert.equal(sends,1);assert.equal(await page.getByRole('heading',{name:'Tokens received.'}).count(),0);
  await page.getByRole('button',{name:'Request quote'}).click();await page.getByRole('button',{name:'Confirm swap with Privy'}).click();await page.getByRole('heading',{name:'Tokens received.'}).waitFor({timeout:15000});assert.equal(sends,2);assert.ok(statusCalls>=2);assert.deepEqual(errors,[]);
  ...
  await page.getByLabel('Token to swap',{exact:true}).selectOption('eth');await page.getByLabel('ETH amount to swap').fill('0.004');await page.getByRole('button',{name:'Request quote'}).click();await page.getByRole('heading',{name:'Controlla lo swap'}).waitFor();assert.equal(swapRequests.at(-1).inputAssetId,'eth');assert.equal(swapRequests.at(-1).amount,'0.004');
  await page.screenshot({path:'artifacts/admin-swap-eth-collaborator-ipad.png',fullPage:true});await page.getByRole('button',{name:'Confirm swap with Privy'}).click();await page.getByRole('heading',{name:'Tokens received.'}).waitFor({timeout:15000});assert.equal(sends,3);await page.getByText('Gas mode: ETH from the shared wallet · ETH fallback.').waitFor();
  ...
  await nvidiaCard.getByRole('button',{name:'Deposit into the slot'}).click();
  await page.getByLabel('NVDAc amount').fill('0.12345678');await page.getByRole('button',{name:'Simulate and confirm deposit'}).click();
  ...
  await page.getByRole('button',{name:'Confirm from my wallet'}).click();await page.waitForFunction(()=>!document.querySelector('.inventory-deposit'));assert.equal(depositSends,1);assert.deepEqual(errors,[]);
