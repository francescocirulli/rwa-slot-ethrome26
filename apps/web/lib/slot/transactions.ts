      ...
      void withWalletAuthorization(request,user,async authorization=>{
        ...
        try {
          const assertAccess=async()=>{
            ...
            if(scope!=='admin'){
              if(['approveBudget','transferERC20','transferERC1155'].includes(op.action)){const player=await slot.reader.walletState(op.address as Address);if(player.busy)throw new SlotError('GamePending','Wait for the spin to settle.',409);}
              return;
            }
            const access=await admin!.assertAction(user,op.action);
