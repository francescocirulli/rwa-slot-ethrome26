...
export function buildAction(reader: SlotReader, account: Address, action: string, inputs: string[]) {
  ...
  if (action === 'approveBudget') {
    ...
    if (!amount || recipient === zeroAddress || recipient.toLowerCase() === account.toLowerCase() || recipient.toLowerCase() === reader.config.address.toLowerCase() || recipient.toLowerCase() === to.toLowerCase()) throw new SlotError('Input', 'Quantità o destinatario non valido.', 400);
    if (!nft && !assetByAddress(to)) throw new SlotError('Asset', 'Token non supportato.', 400);
...
export async function prepareAction(reader: SlotReader, account: Address, action: string, inputs: string[]) {
  ...
  if (action === 'transferERC20') {
    ...
    if (decimals !== asset.decimals) throw new SlotError('Asset', 'Decimali del token non verificati.', 503);
  ...
  if (action === 'transferERC1155') {
    ...
    if (!known && !(await reader.catalog()).some(prize => prize.kind === 2 && prize.token.toLowerCase() === tx.to.toLowerCase() && prize.tokenId === id)) throw new SlotError('Asset', 'Premio NFT non supportato.', 400);
    ...
    if (balance < uint(inputs[3])) throw new SlotError('Balance', 'Quantità NFT insufficiente.', 400);
  }
