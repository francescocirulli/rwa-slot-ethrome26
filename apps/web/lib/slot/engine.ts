import {createWalletClient, encodeFunctionData, parseEventLogs, keccak256, toHex, type Address, type Hash, type Hex, type TransactionReceipt} from 'viem';
...
export function createSlotEngine(reader: SlotReader, backendKey?: Hex, writes:WriteCoordinator=personalWrites()) {
  ...
  const wallet = backend ? createWalletClient({account: backend, chain, transport: rpcTransport(backendRpcUrls)}) : undefined;
