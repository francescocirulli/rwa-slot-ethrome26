...
export function TransactionConfirmation({review,onDecision}:{review:TransactionReview|null;onDecision:(accepted:boolean)=>void}) {
  ...
  }}><span className="tx-eyebrow">IL TUO WALLET PRIVY · BASE</span><h2 id="tx-review-title">Conferma l’operazione.</h2><h3>{review.action==='approveBudget'?'Autorizzazione budget USDC':action?.label||review.action}</h3>
    {asset&&<p><b>{formatUnits(BigInt(review.args[1]),asset.decimals)} {asset.ticker}</b> dal wallet alle riserve della slot.</p>}
    <dl><dt>Wallet che firma</dt><dd><code>{review.address}</code></dd><dt>Contratto destinatario</dt><dd><code>{review.transaction.to}</code></dd>{review.args.map((arg,index)=><div key={index}><dt>{action?.fields[index]||'Budget autorizzato per le giocate'}</dt><dd>{review.action==='approveBudget'?formatUnits(BigInt(arg),6)+' USDC':arg}</dd></div>)}</dl>
