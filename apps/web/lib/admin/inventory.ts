...
export function createInventoryReader(rpcUrl='https://base-rpc.publicnode.com',collection:Address=BASE_PRIZE_COLLECTION,client=createBaseReadClient(rpcUrl)) {
  ...
  async function read(address:Address,contract:Address|null,fundingOnly=false) {
    await check();
    ...
    const [balances,native,contractNative,catalog,owner,pendingOwner]=await Promise.all([
      ...
      contract&&!fundingOnly?client.getBalance({address:contract,blockNumber}).catch(()=>null):null,
      contract?client.readContract({address:contract,abi:slotAbi,functionName:'getPrizeCatalog',blockNumber}).catch(()=>null):null,
      fundingOnly?null:client.readContract({address:collection,abi:prizeCollectionAbi,functionName:'owner',blockNumber}).catch(()=>null),
      fundingOnly?null:client.readContract({address:collection,abi:prizeCollectionAbi,functionName:'pendingOwner',blockNumber}).catch(()=>null),
    ]);
    ...
    const freeSpins=contract&&!fundingOnly?await client.readContract({address:contract,abi:slotAbi,functionName:'freeSpins',args:[address],blockNumber}).catch(()=>null):null;
    ...
    return {address,contract,block:blockNumber.toString(),updatedAt:Date.now(),eth:formatUnits(native,18),contractEth:contractNative===null?null:formatUnits(contractNative,18),assets,nfts,freeSpins:freeSpins?.toString()??null,catalogAvailable:!!catalog,
      funding,scope:fundingOnly?'funding':'full',collection:{address:collection,owner,pendingOwner,canMint:owner?.toLowerCase()===address.toLowerCase(),canAcceptOwnership:pendingOwner?.toLowerCase()===address.toLowerCase()}};
  };
  return (address:Address,contract:Address|null,fundingOnly=false)=>{
    const key=address.toLowerCase()+':'+contract?.toLowerCase()+':'+fundingOnly;
    const running=pending.get(key);if(running)return running;
    const request=read(address,contract,fundingOnly).finally(()=>pending.delete(key));pending.set(key,request);return request;
  };
}
