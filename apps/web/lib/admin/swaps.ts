...
export function createSwapChain(rpcUrl='https://base-rpc.publicnode.com'):SwapChain {
  ...
  async function check(){if(await client.getChainId()!==8453)throw new SlotError('Chain','Il nodo RPC non è su Base.',503);}
  return {
...
export function swapError(error:unknown) {
  ...
  return new SlotError('SwapProvider','Stato non verificabile. Aggiorna prima di ripetere l’operazione.',503);
}
export function createSwapService({client,walletService,admin,chain=createSwapChain(),getQuote=createLifiClient(),coordinator=createWriteCoordinator(),now=Date.now}:{client:PrivyClient;walletService:WalletService;admin:AdminAccessService;chain?:SwapChain;getQuote?:(intent:SwapIntent)=>Promise<LifiQuote>;coordinator?:WriteCoordinator;now?:()=>number}) {
  ...
  async function access(user:Identity,write=false){const value=await admin.resolve(user);if(write&&value.role!=='owner'&&!value.swapEnabled)throw new SlotError('SwapAccess','Il proprietario deve aggiornare i tuoi permessi nella pagina Wallet admin per abilitare LI.FI.',403);return value;}
  async function resolveSubmission(walletId:string,submission:SubmittedSpin,address:Address,fromBlock?:string,onProgress?:(block:string)=>void){
    if(submission.transactionId){
      ...
      if(tx.wallet_id!==walletId||tx.caip2!=='eip155:8453')throw new SlotError('SwapAccess','Transazione non corrispondente al wallet condiviso.',403);
      ...
      if(['execution_reverted','failed'].includes(tx.status))throw new SlotError('TransactionFailed','La transazione non è riuscita. I costi di gas già sostenuti non vengono restituiti.',409);
      if(tx.transaction_hash&&/^0x[\da-f]{64}$/i.test(tx.transaction_hash))submission.hash=tx.transaction_hash as Hash;
      if(tx.status==='replaced'&&!submission.hash)throw new SlotError('SwapPending','Transazione sostituita: attendi il nuovo riferimento prima di riprovare.',409);
    }
    if(!submission.hash&&submission.userOperationHash&&fromBlock){
      ...
      if(event.hash){submission.hash=event.hash;if(event.success===false)throw new SlotError('TransactionFailed','L’operazione con gas USDC è stata annullata onchain.',409);}
    ...
    if(receipt&&submission.userOperationHash){
      ...
      if(events.length!==1)throw new SlotError('SwapReceipt','Ricevuta dell’operazione Privy non ancora verificabile.',409);
      if(!events[0].args.success)throw new SlotError('TransactionFailed','L’operazione con gas USDC è stata annullata onchain.',409);
    }
    return receipt;
  }
  ...
  return {
    async quote(user:Identity,assetId:unknown,amount:unknown,inputAssetId:unknown='usdc'){
      ...
      if(!asset||!inputAsset)throw new SlotError('SwapAsset','Scegli USDC o ETH in ingresso e uno dei sei premi in uscita.',400);
      ...
      if(operations.size>=256)throw new SlotError('SwapBusy','Troppe richieste in attesa.',503);
    ...
    async execute(user:Identity,authorization:WalletAuthorization,id:unknown,confirm:unknown){
      ...
      if(op.expiresAt<=now()){op.stage='expired';throw new SlotError('SwapExpired','Quotazione scaduta. Richiedine una nuova.',409);}
      op.stage='submitting';let sending=false;
      try {
        await coordinator.acquire(wallet.id,op.id,()=>reconcile(op));
        if(!walletService.sendOwned)throw new SlotError('Config','Invio Privy non configurato.',503);
        if(await chain.balance(op.address,op.inputAssetId)<BigInt(op.intent.amount))throw new SlotError('SwapBalance','Saldo insufficiente per lo scambio.',400);
        ...
        if(op.step==='swap'&&op.inputAssetId==='usdc'&&allowance<BigInt(op.intent.amount))throw new SlotError('SwapAllowance','L’autorizzazione USDC è cambiata. Richiedi una nuova quotazione.',409);
        if(op.step==='swap'){
          const fresh=await getQuote(op.intent);
          if(BigInt(fresh.minimum)<BigInt(op.quote.minimum))throw new SlotError('SwapPriceChanged','Il minimo ricevuto è diminuito. Richiedi una nuova quotazione e confermala.',409);
          op.quote=fresh;
        }
        const assertAccess=async()=>{const current=await access(user,true);if(current.wallet.id!==wallet.id)throw new SlotError('AdminAccess','Il wallet condiviso è cambiato.',403);if(op.expiresAt<=now())throw new SlotError('Cancelled','Quotazione scaduta prima dell’invio. Richiedine una nuova.',409);};
    ...
    async status(user:Identity,id:string|null,actionId:string|null,hash:string|null=null,userOperationHash:string|null=null,fromBlock:string|null=null):Promise<SwapView>{
      ...
      if(actionId&&/^[a-zA-Z0-9_-]{8,128}$/.test(actionId))submission.transactionId=actionId;
        ...
        const tx=await chain.transaction(hash as Hash);if(tx.from.toLowerCase()!==wallet.address.toLowerCase())throw new SlotError('SwapAccess','Transazione non appartenente al wallet condiviso.',403);submission.hash=hash as Hash;
      }else throw new SlotError('SwapUnknown','Riferimento non recuperabile dopo il riavvio. Controlla le transazioni del wallet su BaseScan e Privy prima di un nuovo swap.',404);
      ...
      throw new SlotError('SwapReceipt','La ricevuta non contiene uno swap o un’autorizzazione LI.FI riconoscibile. Verifica manualmente.',409);
      };
