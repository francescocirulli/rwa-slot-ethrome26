...
export function createAssetsHandler({admin,walletService,swaps,inventory,contract,origin}:{admin?:AdminAccessService;walletService?:WalletService;swaps?:ReturnType<typeof createSwapService>;inventory:ReturnType<typeof createInventoryReader>;contract:()=>Address|null;origin:string}){
  return async function handle(request:Request,action:'inventory'|'quote'|'execute'|'status'){
    ...
    try{
      ...
      if(request.method!==(write?'POST':'GET'))throw new SlotError('Method','Method not allowed.',405);
      if(write&&(request.headers.get('origin')!==new URL(origin).origin||request.headers.get('x-slot-request')!=='1'))throw new SlotError('Origin','Invalid origin.',403);
      if(!admin||!walletService||!swaps)throw new SlotError('Config','Shared wallet not configured.',503);
      ...
      if(action==='inventory')return reply({...await inventory(access.wallet.address as Address,contract(),new URL(request.url).searchParams.get('scope')==='funding'),swapEnabled:access.role==='owner'||!!access.swapEnabled,mintEnabled:access.role==='owner'||!!access.mintEnabled,canManageOwnership:access.role==='owner'});
