...
export function createAdminHandler({service,walletService,origin,readBalance}:{service?:AdminService;walletService?:WalletService;origin:string;readBalance:(address:string)=>Promise<Balance>}) {
  return async function handle(request:Request,action:'account'|'create'|'member'|'proof') {
    ...
    try {
      if(request.method!==(action==='account'?'GET':'POST'))throw new SlotError('Method','Metodo non consentito.',405);
      if(action!=='account'&&(request.headers.get('origin')!==new URL(origin).origin||request.headers.get('x-slot-request')!=='1'))throw new SlotError('Origin','Origine non valida.',403);
      const token=request.headers.get('authorization')?.match(/^Bearer ([^ ]+)$/)?.[1];
      if(!token||token.length>16000)throw new SlotError('Auth','Accedi con il tuo account Privy.',401);
      if(!service||!walletService)throw new SlotError('Config','Privy non configurato.',503);
      ...
      if(action!=='account') {
        ...
        if(!input||input.confirm!==true)throw new SlotError('Consent','Conferma prima questa operazione.',400);
