...
export function entitiesFor(config:ArkivConfig, source:{chainId:number;address:`0x${string}`}, game:ConfirmedGame, timestamp:bigint, season:Season|null, seasonStartTime:bigint):CreateEntityParameters[] {
  ...
  const baseAttributes = {project:str(config.project),schema:i32(1),sourceChain:u64(source.chainId),slot:addr(source.address),gameId:u256(game.id),player:addr(game.player),playedAt:u64(timestamp),won:bool(game.won),points:i32(pointsFor(game.won,game.matchCount)),matches:i32(game.matchCount),symbol:i32(game.winningSymbol),prizeKind:i32(game.payout?.kind || 0)};
  ...
  if (season && timestamp >= seasonStartTime) creates.push({...history,attributes:{...baseAttributes,kind:str('season-spin'),schedule:str(schedule),season:u64(BigInt(season.id)),seasonEnd:u64(season.endBlock)},payload:jsonToPayload({transactionHash:game.transactionHash}),expires:ExpirationTime.atBlock(season.endBlock)});
  return creates;
}
export function createArkivStore(config: ArkivConfig, reader: SlotReader) {
  ...
  const schedule = `${config.anchor}:${config.seasonBlocks}`;
  const scope = [eq('project',str(config.project)),eq('schema',i32(1)),eq('sourceChain',u64(reader.config.chainId)),eq('slot',addr(reader.config.address))];
  ...
  async function contributions(season:Season, block:bigint, startTime:bigint) {
    ...
    const q = query('season-spin',[eq('schedule',str(schedule)),eq('season',u64(BigInt(season.id))),gte('playedAt',u64(startTime)),eq('seasonEnd',u64(season.endBlock))],block);
    for await (const entity of q) {
      const a = entity.attributes;
      if(a.player?.type !== 'addr' || a.gameId?.type !== 'u256' || a.points?.type !== 'i32' || a.won?.type !== 'bool' || entity.expiresAt !== season.endBlock) throw new Error('Invalid season entity');
      entries.push({gameId:String(a.gameId.value),player:a.player.value,points:a.points.value,won:a.won.value,expiresAt:entity.expiresAt});
    }
    return entries;
  }
  async function hasGame(id:bigint) {return (await query('spin',[eq('gameId',u256(id))]).fetch()).entities.length > 0;}
