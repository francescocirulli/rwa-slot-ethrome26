...
export const NFT_PRIZES=[{symbol:0,name:'Magnet',key:'MAGNET'},{symbol:3,name:'Gadget',key:'GADGET'},{symbol:8,name:'ENS Registration',key:'ENS_REGISTRATION'},{symbol:9,name:'Urbe Hub Day Pass',key:'URBE_HUB_DAY_PASS'},{symbol:10,name:'T-shirt',key:'SHIRT'}] as const;
export const PRIZE_LABELS=['MAGNET','FREE SPIN','NVIDIA','GADGET','SPACEX','APPLE','ALPHABET','AMAZON','ENS','URBE PASS','T-SHIRT','GOLD','SYMBOL 12','SYMBOL 13','SYMBOL 14','SYMBOL 15'] as const;
...
export function assetUnits(value:unknown,decimals:number):bigint {
  ...
  const amount=parseUnits(value,decimals);if(amount<=0n||amount>=2n**256n)throw new Error('Invalid amount.');return amount;
}
