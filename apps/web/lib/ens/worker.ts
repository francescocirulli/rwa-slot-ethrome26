...
export function createEnsWorker({fromBlock,pageBlocks=1000n,head,events,complete}:{fromBlock:bigint;pageBlocks?:bigint;head:()=>Promise<bigint>;events:(from:bigint,to:bigint)=>Promise<Source[]>;complete:(claim:Source)=>Promise<void>}){
  ...
  function schedule(){if(stopped)return;timer=setTimeout(()=>{void tick().catch(()=>{}).finally(schedule);},catchingUp?1000:15000);timer.unref();}
  return {tick,start(){if(timer||stopped)return;void tick().catch(()=>{}).finally(schedule);},stop(){stopped=true;if(timer)clearTimeout(timer);},state:()=>({cursor,pending:pending.size,catchingUp})};
}
