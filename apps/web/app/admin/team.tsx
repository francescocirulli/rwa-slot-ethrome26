...
export function AdminTeam({account,onRefresh}:{account:AdminAccountView;onRefresh:()=>void}) {
  ...
  return <section className="admin-card admin-team"><div className="card-heading"><h2>Un wallet, il vostro team.</h2><span className="real-badge">{account.role==='owner'?'PROPRIETARIO':'COLLABORATORE'}</span></div>
    ...
    {account.role==='owner'?<><h3>Collaboratori autorizzati</h3>{account.members.length?<ul className="team-members">{account.members.map(item=><li key={item.userId}><div><code>{item.userId}</code><span>{item.enabled?'Permessi aggiornati · swap e mint':'Permessi da aggiornare per swap, slot o mint NFT'}</span></div><div>{!item.enabled&&<button className="admin-text" disabled={busy} onClick={()=>setReview({id:item.userId,remove:false})}>Aggiorna permessi</button>}<button className="admin-text" disabled={busy} onClick={()=>setReview({id:item.userId,remove:true})}>Rimuovi accesso</button></div></li>)}</ul>:<p className="fine-print">Nessun collaboratore. L’altra persona deve accedere all’admin e inviarti il suo codice account.</p>}
