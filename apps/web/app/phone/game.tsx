...
export function PhoneGame({session,portfolio,onWallet}:{session:SessionView;portfolio:Portfolio|null|undefined;onWallet:()=>void}){
 return <section className="phone-card phone-play-card"><span className="eyebrow">THE SLOT IS LINKED · BASE</span><h2>Ready at the iPad.</h2>
  ...
  <p>{session.playGrant?.active?'Paid spins are enabled for this iPad session.':portfolio?.allowance&&BigInt(portfolio.allowance)>0n?'Your USDC limit is already approved. Enable this iPad from Wallet without another USDC approval.':'Set up paid spins from Wallet when you are ready.'}</p>
