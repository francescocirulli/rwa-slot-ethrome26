...
import {PhoneApproval,type PlayPermission} from './approval';
...
export function PhoneWallet({wallet,transaction,permission,reload,loading=false}:{wallet:NonNullable<AccountView['wallet']>;transaction:PhoneTransaction;permission?:PlayPermission;reload:()=>void;loading?:boolean}) {
  const [selection,setSelection]=useState<Holding|null>(null),[recipient,setRecipient]=useState(''),[amount,setAmount]=useState(''),[error,setError]=useState(''),[copied,setCopied]=useState('');
  ...
  return <>
    <section id="wallet-funding" tabIndex={-1} className="phone-card"><div className="wallet-heading"><span className="eyebrow">YOUR WALLET</span><span className="base-badge">● BASE</span></div>
    ...
    <PhoneApproval portfolio={portfolio} transaction={transaction} permission={permission} reload={reload} stale={wallet.balance.stale}/>
