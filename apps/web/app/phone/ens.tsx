...
export function PhoneENS({address,onChanged}:{address:string;onChanged:()=>void}){
 ...
 return <section className="phone-card" aria-label="ENS names on Sepolia">
  ...
  {view?.names.map(item=><div className="ready-card" key={item.name}><b>{item.name}</b><p className="small">{namesState==='ready'?'Registered on Sepolia · Owned by your wallet':'Last verified on Sepolia'} · Expires {new Date(Number(item.expiry)*1000).toLocaleDateString()}{item.resolvedAddress?.toLowerCase()!==address.toLowerCase()?' · Address record differs from your wallet':''}</p><button className="phone-text" onClick={()=>void run(()=>navigator.clipboard.writeText(item.name))}>Copy name</button><a className="phone-chain-link" href={'https://explorer.ens.dev/'+encodeURIComponent(item.name)} target="_blank" rel="noreferrer">ENS Explorer ↗</a></div>)}
