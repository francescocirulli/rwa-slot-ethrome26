...
export function PhoneENS({address,onChanged}:{address:string;onChanged:()=>void}){
 ...
 async function api(body?:unknown,authorize=false){
  ...
  if(!response.ok)throw Error(data.error||'Registration is temporarily unavailable.');return data;
 }
 ...
 async function send(){
  ...
  if(r.address.toLowerCase()!==address.toLowerCase()||r.expires<Date.now()||r.registrationPayer!=='backend')throw Error('Review expired. Please continue again.');
 ...
 return <section className="phone-card" aria-label="ENS names on Sepolia">
  ...
  {view?.claims.filter(c=>!c.completed).map(c=><div className="receive-box" key={c.id}><b>{c.name}</b>
   ...
   {c.stage==='voucher'&&!pending&&!review&&<button className="phone-secondary" disabled={busy} onClick={()=>void run(()=>prepare(c.id))}>Continue</button>}
  </div>)}
  {view&&!view.claims.some(c=>!c.completed)&&!pending&&<form onSubmit={event=>{event.preventDefault();void run(async()=>{const claim=await apiRef.current({action:'reserve',label,confirm:true});await refresh();await prepare(claim.id);});}}>
