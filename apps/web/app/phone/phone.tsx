...
function Phone() {
  ...
  return <Shell>
    ...
    {connectedSession && session && <>
      {session.state === 'active' && account?.wallet?.address.toLowerCase()===session.address?.toLowerCase() && <PhoneGame session={session} api={api} transaction={transaction} onSession={value => {if (latest.current.session?.id === value.id && deadline.current > Date.now()) apply(value);}}/>}
