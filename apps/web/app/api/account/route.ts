...
import {prizeCollectionAddress} from '@/lib/prize-collection';
...
export async function GET(request: Request) {
  if (!handler) {
    ...
    handler = createAccountHandler(appId && secret ? createWalletService(appId, secret) : undefined, createBalanceReader(process.env.BASE_RPC_URL), createPortfolioReader(createInventoryReader(process.env.BASE_RPC_URL,prizeCollectionAddress(process.env.SLOT_PRIZE1155_ADDRESS)),getSlotEngine));
  }
