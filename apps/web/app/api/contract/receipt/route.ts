...
export async function GET(request: Request) {
  ...
  if (!hash || !/^0x[0-9a-fA-F]{64}$/.test(hash)) return Response.json({error: 'Invalid hash.'}, {status: 400});
  try {
    const slot = getSlotEngine(); if (!slot) return Response.json({error: 'Contract not configured.'}, {status: 503});
